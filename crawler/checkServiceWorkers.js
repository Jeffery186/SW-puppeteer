...
async function createServer(source, filename){
    functonsDef = `
        (function(console){
    ...
    server = http.createServer((req, res) => {
        ...
        res.end("<script>" + functonsDef + content + "</script>");
    });
...
async function program(){
    ...
    for(let i = 0; i < ServiceWorkers.length; i++){
        ...
        await createServer('ServiceWorkers/' + ServiceWorkers[i], ServiceWorkers[i].split('-')[0]);
        await page.goto(`http://${hostname}:${port}/`);
        await waitAndClose(15000);
    }
    await browser.close();
}
