...
plot "data.dat" using (($2 / totalSites) * 100):xtic(1) title "sites with SW" fs solid lc rgb "blue",\
     "" using ($0 - 0.1):((($2 / totalSites) * 100) ):(sprintf("%3.2f%",($2/totalSites)*100)) with labels font ",10" rotate by 90 offset 0,1.5 notitle,\
     "" using (($4 / swSites) * 100) title "sites with SW fetching ads" fs solid lc rgb "red",\
     "" using ($0 + 0.2):((($4 / swSites) * 100)):(sprintf("%3.2f%",($4/swSites)*100)) with labels font ",10" rotate by 90 offset 0,1.5 notitle